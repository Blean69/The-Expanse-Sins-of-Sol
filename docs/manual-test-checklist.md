...
## U14 — Hull visibility, restored detail and firing arcs

Use only `expanse_update14` in a new game; record ZIP hash, faction, ship levels, camera/settings and logs. All results begin NOT RUN.

1. **U14-1 appearance:** apply without errors; inspect the Scirocco exterior from both sides, above, below and grazing angles. Compare Raptor/Pella hull bevels and all four engine bells with0.13. Check attached PDC/rail geometry, orange/silver colors, Pella eagle, launches and exhaust positions.
2. **U14-2 arcs/interception:** move hostile ships and torpedoes around every Raptor/Pella mount at low/high elevation and arc boundaries. Obstructed guns should hold fire while clear mounts track and intercept. Watch for shots through the hull, premature firing within aim tolerance, muzzle displacement and unacceptable blind spots. Measure unchanged individual damage/cadence; cover both ships and different yaw directions.
3. **U14-3 persistence/performance:** save/reload in combat, then measure one capital and the intended mixed fleet using matching camera, graphics settings and target activity. Record frame times and logs. Prior low-poly fleet performance does not certify restored geometry. Repeat earlier pending gameplay gates separately.
