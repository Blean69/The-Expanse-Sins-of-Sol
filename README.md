...
**Current local candidate: [0.27.4 Hephaestus Repair](docs/update27_4.md).** Corrects the engine-rejected Hephaestus target type, four invalid station-control flags and Donnager’s foreign prerequisite; replaces Murphy’s printed aft stub with a textured Tachi Epstein drive. The user confirmed that 0.27.3 Menu Battle crashes before reaching the menu, so this candidate has no custom menu. Offline validation passes; runtime closure and Pella’s debug-spawn GUI error remain unconfirmed. Enable the repair alone. Frozen predecessors are retained, including the [0.26 fallback](docs/update26.md). No installation or game launch performed. Earlier development notes below are historical.
