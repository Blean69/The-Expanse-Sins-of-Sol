...
**Current local candidate: [0.27 Fleet Expansion](docs/update27.md).** Adds Nathan Hale, Munroe, Hephaestus, Dark Star and a Laconian frigate; moves Gathering Storm procurement to OPA, replaces Behemoth art and grafts detailed donor Epstein drives into printed engine stubs. Regular faction access and the combined Sandbox are separate. Three diagnosed loading/runtime assertions have offline repairs; new game, save/reload and multiplayer testing remain pending. **Preserved rollback: [0.26 Fleet Polish](docs/update26.md).** The user's Composite receive/equip observation applies only to the separate inert currency laboratory. Package hashes and validation are recorded under `audit/update27/`; prior development notes below are historical. Nothing was installed, launched or published.
