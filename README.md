...
**Current local candidate: [0.27.3 Load Repair](docs/update27_3.md), with an [optional menu battle variant](docs/menu27_3.md).** Repairs confirmed 0.27.1 action-level, finite-item, Behemoth role, Foehammer clearance and railgun muzzle-socket errors. Both packages pass offline checks; the reported all-ships crash and final empty-resource error are not yet confirmed resolved. Test the regular repair first, then the menu variant, enabling only one package at a time. Model polish is deferred. **Preserved fallback: [0.26 Fleet Polish](docs/update26.md).** Frozen 0.27/0.27.1 and the undelivered 0.27.2 menu preview are retained for diagnosis; 0.27.2 lacks the new repairs. No installation or game launch performed. Earlier development notes below are historical.
