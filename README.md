...
**Staged candidate: [0.19 Fleet balance and extracted hulls](docs/update19.md).** Standardized PDCs, slower railguns, faster double-damage torpedoes with30-second fuel, Europa's Bane, Artemis civilian tender/trade hull and Le Guin native derelict. Three shared identities and the small Sol scenario remain.171 definition-schema checks and asset/action/arithmetic checks passed offline; gameplay/save/reload/multiplayer testing is pending. Windows PDC sound was confirmed audible by the user; no audio changes. **Preserved rollback: [0.18 Sol Gate1](docs/update18.md)**; **accepted working fleet: [0.17](docs/update17.md)**. No remote push, publication or automatic installation under the current milestone. ZIP/hash details are in `audit/update19/package-summary.json`.
