...
**Current local candidate: [0.27.1 Runtime Repair](docs/update27_1.md).** Repairs the 0.27 unit-tag overflow that broke native classifications/item restrictions, missing PDC rendering sockets and missing experience effects. The user reported a crash on 0.27; it is retained for diagnosis, not recommended for play. New equipment/tag/socket checks pass offline; post-fix gameplay and multiplayer remain untested. **Preserved fallback: [0.26 Fleet Polish](docs/update26.md).** No installed files or saves were changed. The optional Expanse menu battle is documented as a follow-up. Earlier development notes below are historical.
