...
This record traces assets supplied for the project, source licenses, reported permissions and derivative changes. Earlier milestone notes describe the state at the time; local asset processing does not establish permission for public redistribution. Machine-specific directories are intentionally omitted.
...
a locally installed copy of the game. No NASA textures were downloaded or incorporated;
...
- Received for the project and inspected 2026-09-12. Sketchfab's page returned HTTP 403 during verification; license identification is from the supplied package, not an independently observed current download page.
...
Supplied archive: `Rocinante_(The_Expanse).zip`. Permission to use the model was reported during intake; this is **reported permission**, not an independently verified public license. The archive contains no license file; creator, source URL, exact license terms and attribution requirements remain unspecified. No public distribution or publication was performed. Do not substitute the original Tachi author's CC-BY license for this separate asset.
...
Hero archive SHA-256: `2fef806f80c65f138eed9b877c6cd1ae98f12e597856b96cfb1c6676a16af3e6`. Preserved package: `assets/derived/geometry03-b/hero-original/Rocinante_(The_Expanse).zip`; extracted files: sibling `hero-master/`, in a local, ignored worker directory. Individual master hashes and relative-animation measurements: `audit/geometry03-b/hero-asset-audit.json`.
...
Six additional WAV recordings were supplied for local integration. Audio creator, exact license and upstream recording attribution remain unspecified; model permission is not substituted for audio permission. Originals are retained unchanged under `assets/original/voice08`, normalized editable WAVs under `assets/derived/voice08/normalized-wav`, and generated game audio under `build/voice08/game/sounds`. No audio was uploaded or published.
...
New sprites are renders of these derivatives and inherit their source records. Editable models, source audio, rendered asset imagery and generated game packages stay outside the source push. Supplied captain speech is reused for Morrigan without railgun/hammers lines; no new audio or voice identity is introduced.

## Raptor, Pella, Scirocco and Sunflare — update 0.12

The following files were supplied for local conversion. Creator, source-page identity and exact license were not supplied for the STL files. The Sunflare archive contains no separate creator/license record; its exporter metadata identifies OpenSceneGraph, which is not treated as an author credit. No wider redistribution license is invented. Originals and extracted masters remain unchanged.

- Raptor/Pella: `xxx_-_raptor_whole.stl`, SHA-256 `c06878f57e5b9f04043793226f8c6a06b082848273ea6fc9e9369f474c8dedd6`. Original 1,513,390 triangles, 291 components, no UV/material/animation. Separate gray/orange MCRN and metallic silver/charcoal Free Navy derivatives; supported donor PDCs, forward launcher collars, proper normalization and game/UI conversion.
- Scirocco: `xxx_-_scirocco_whole.stl`, SHA-256 `ea72a0b5cb4f57b23851335706f898fd45d9788f78ec06fa64ae272bee7ca64a`. Original 677,402 triangles, 159 components, no materials/textures/animations. Derivative 89,519 assembled triangles with twelve credited donor PDCs, ten supported launcher collars and an actual source rail assembly separated from the hull onto an extended bearing.
- Sunflare: `Series_6_Sunflare_Racing_Pinnace_[The_Expanse].zip`, SHA-256 `0f861de9e69b6b0cf3959e083b45571f9879a35cc6b2bfb62f50e6bf4f103e86`. Original glTF 60,635 triangles, one mesh/material, six transform nodes and four 4096-square textures; no animations. Derivative 17,741 triangles, source UV/livery retained, four 2048-square DDS maps, explicit isolated tangent/normal repairs and one measured central engine. Source Razorback markings are retained.

All added PDCs inherit the Tachi source attribution and permission record above. New captain playback reuses the previously supplied generic Martian recordings; no new voice or sound file is introduced. Actual-model UI inherits the relevant model source record.

Pella eagle references: `codex-clipboard-872e1a76-e69c-4245-867e-02fb6414a1d3.png` (TV placement) and `codex-clipboard-3fac540f-2f08-44e8-a6b3-a3e204fac1a9.png` (close-up) were supplied for emblem integration. The resulting clean raster is a generated interpretation of that supplied design, mapped onto existing forward hull faces in a separate Pella derivative. It is not asserted to be an official production texture. See `audit/update12-a` for reference/output hashes and placement evidence. No separate emblem license or creator endorsement is claimed.

Editable assets live in the isolated workers' `assets/derived/update12-a`, `update12-b` and `update12-c`; preserved source copies are under their `assets/original/update12-*` and Sunflare `assets/source/update12-sunflare`. Compiled game assets and render outputs live in the separate `build/update12-*` trees. Source scripts/text audits may be pushed; supplied binaries, textures, audio, generated emblem, rendered imagery and model/game derivatives remain ignored and are not included in the source push.

## Model and audio polish — update 0.13

Scirocco reference imagery was supplied as `paul-kiesling-display-2.jpg`, `paul-kiesling-display-3.jpg`, `paul-kiesling-material-1.jpg` through `-4.jpg`, and `paul-kiesling-4.jpg`. The visible artwork credits **Paul Kiesling**. The derivative maps colors from these references; an exact original texture license was not supplied. These are compressed display/contact sheets, and the supplied STL has no matching original UVs. The derivative uses a reference-guided generated charcoal panel texture and newly mapped red/orange bands; it is not represented as the original artist's complete skin or a pixel-perfect UV transfer. Hashes and exact generation/source records are in `audit/update13-a` and `audit/update13/checkpoint.json`. The twelve Tachi/Pella-family PDC assemblies retain their earlier source attribution; their visual scale and fixed supports are adapted to the Scirocco hull. No extra weapon is created.

Morrigan uses existing Martian materials on new clipped paint regions of the derivative. Raptor/Pella shading frames change without changing source positions, UVs, silhouette or emblem mapping. Sunflare receives a proper whole-model longitudinal roll, preserving original texture pixels and proportions; source and original/master files remain untouched. Editable and compiled files remain separate in the three isolated `visual13-*` worktrees.

PDC sound derivatives use only the previously supplied and preserved `PDC.mp3` / normalized WAV. Four source windows starting at 0.06, 0.25, 0.44 and 0.63 seconds produce 0.20-second reports, with 4ms fade-in, 25ms fade-out and a further −10dB gain. Editable WAVs are under `assets/derived/update13-audio`; game Vorbis files and existing-field `.sound` profiles are under `build/update13-audio/game`. Original and previous normalized audio remain unchanged. These edits do not authorize or perform publication of the recording. As before, source scripts/text records may be pushed, while model/audio/texture derivatives and packages are ignored.

## Hull corrections — update 0.14

The same supplied Scirocco and Raptor STL source records apply. Scirocco's derivative face orientation is repaired while preserving the existing reference-guided paint and credited donor weapons. Raptor/Pella recover more actual original hull and engine detail through a new derivative reduction; existing materials and the previously generated Pella emblem remain in use. No original archive, STL, extracted master, texture source or recording is edited. New meshes and review renders remain local ignored assets in isolated `visual14-*` worktrees; this source push contains no model or media derivatives.

## UNN Truman — update0.15

The supplied `truman_class.zip` has SHA-256 `a7ad8ce87bc9ac6d04b1873bfb3a2814ab93a7453f52a55a5f3d2937a191941c`. Archive attribution identifies **mohamedhussien**, [Truman class on Sketchfab](https://sketchfab.com/3d-models/truman-class-ec7e5f790d2940e489bf28c6e6d4427e), licensed [Creative Commons Attribution4.0](https://creativecommons.org/licenses/by/4.0/). Original textured source retained; derivative rotates/scales the model, separates two existing railgun assemblies, adds eighteen credited Tachi-derived PDC battery representations, repairs float32-degenerate slivers, and converts textures/mesh formats. Actual-model UI renders inherit these sources. No endorsement is implied. Original/source/derived media and game package remain outside the source-only push. See `audit/update15-truman` for exact final counts, source hashes, conversion and validation records.
