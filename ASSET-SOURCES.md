...
The user supplied the following files and requested local conversion. Creator, source-page identity and exact license were not supplied for the STL files. The Sunflare archive contains no separate creator/license record; its exporter metadata identifies OpenSceneGraph, which is not treated as an author credit. No wider redistribution license is invented. Originals and extracted masters remain unchanged.
...
Pella eagle reference: the user supplied `codex-clipboard-872e1a76-e69c-4245-867e-02fb6414a1d3.png` (TV placement) and `codex-clipboard-3fac540f-2f08-44e8-a6b3-a3e204fac1a9.png` (close-up) and explicitly requested emblem integration. The resulting clean raster is a generated interpretation of that supplied design, mapped onto existing forward hull faces in a separate Pella derivative. It is not asserted to be an official production texture. See `audit/update12-a` for reference/output hashes and placement evidence. No separate emblem license or creator endorsement is claimed.
...
Scirocco reference imagery was supplied by the user as `paul-kiesling-display-2.jpg`, `paul-kiesling-display-3.jpg`, `paul-kiesling-material-1.jpg` through `-4.jpg`, and `paul-kiesling-4.jpg`. The visible artwork credits **Paul Kiesling**. The user requested mapping colors from these references; an exact original texture license was not supplied. These are compressed display/contact sheets, and the supplied STL has no matching original UVs. The derivative uses a reference-guided generated charcoal panel texture and newly mapped red/orange bands; it is not represented as the original artist's complete skin or a pixel-perfect UV transfer. Hashes and exact generation/source records are in `audit/update13-a` and `audit/update13/checkpoint.json`. The twelve Tachi/Pella-family PDC assemblies retain their earlier source attribution; their visual scale and fixed supports are adapted to the Scirocco hull. No extra weapon is created.
