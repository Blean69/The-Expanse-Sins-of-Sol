...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update12-a'/VARIANT;BUILD=ROOT/'build/update12-a'/VARIANT;AUD=ROOT/'audit/update12-a'/VARIANT;MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
...
for kind in ['hull','pdc_base','pdc_barrel']:
...
tex=OUT/'texture-sources';tex.mkdir(exist_ok=True);dest=BUILD/'game/textures';dest.mkdir(parents=True,exist_ok=True);defs={};colors=([[61,71,79,255],[166,41,14,255],[19,22,24,255]] if VARIANT=='raptor' else [[171,179,186,255],[99,110,120,255],[14,17,19,255]])
for name,color in [(PREFIX+'_nrm',[128,128,255,255]),(PREFIX+'_orm',([255,180,100,255] if VARIANT=='raptor' else [255,77,209,255])),(PREFIX+'_msk',[0,0,0,0])]+[(f'{PREFIX}_mat_{i}_clr',color) for i,color in enumerate(colors)]:Image.new('RGBA',(16,16),tuple(color)).save(tex/(name+'.png'))
for i in range(3):defs[f'{PREFIX}_mat_{i}']={'version':1,'base_color_texture':f'{PREFIX}_mat_{i}_clr','occlusion_roughness_metallic_texture':PREFIX+'_orm','normal_texture':PREFIX+'_nrm','mask_texture':PREFIX+'_msk','emissive_factor':0.}
if VARIANT=='pella' and (tex/'expanse12_pella_free_navy_clr.png').exists():
 defs['expanse12_pella_free_navy']={'version':1,'base_color_texture':'expanse12_pella_free_navy_clr','occlusion_roughness_metallic_texture':PREFIX+'_orm','normal_texture':PREFIX+'_nrm','mask_texture':PREFIX+'_msk','emissive_factor':0.}
for inp in tex.glob('*.png'):
 nrm=inp.stem.endswith('_nrm');cmd=[str(wine),str(MAIN/'.tools/texconv.exe'),'-f','BC5_SNORM'if nrm else'BC7_UNORM','-y','-m','0','-nogpu','--single-proc','-o','Z:'+str(dest)]+(['--x2-bias']if nrm else['-bc','q'])+(['-w','1024','-h','1024'] if 'free_navy'in inp.stem else[])+['Z:'+str(inp)]
...
base=MAIN/'build/experiments/expanse_update11';dm=json.loads((base/'mesh_materials/expanse03_pdc_0_base_mcrn_tachi_material.mesh_material').read_text());defs['mcrn_tachi_material']=dm
