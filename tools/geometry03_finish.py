...
out=ROOT/'assets/derived/geometry03-b/corvette';build=ROOT/'build/geometry03-b/corvette';audit=ROOT/'audit/geometry03-b';sources=read(out/'retained-source-frames.json');meta=read(audit/'mount-metadata.json');dest=build/'game/meshes';jdest=build/'repaired-json';dest.mkdir(parents=True,exist_ok=True);jdest.mkdir(exist_ok=True)
...
base=read(Path(meta['source_root'])/'build/compiler-json/mcrn_corvette_baseline.mesh_json');original=np.array([x['p'] for x in base['non_skinned_vertices']])[base['vertex_indices']];actual=np.concatenate(allworld);assert len(actual)//3==meta['triangle_total'];d2,_=cKDTree(actual).query(original);d=d2;assert d2.max()<2e-5
...
dr.text((25,18),'CORVETTE0.3 / SUPPORTS AND TARGETED INNER SHELLS / RUNTIME NOT RUN',fill='white');dr.text((25,44),'Gold yaw/base; cyan pitch/barrel; gray fixed hull/linkage. Left ship X/Z; right ship Y/Z.',fill='white');dr.text((25,850),f'Original14,622 triangles retained; {meta["triangle_total"]} total including targeted backing faces and physical supports.',fill='white');im.save(audit/'six-pdc-mounts.png')
