...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update26-storm';BUILD=ROOT/'build/update26-storm';GAME=BUILD/'game';AUD=ROOT/'audit/update26-storm';BASE=MAIN/'build/experiments/expanse_update25'
...
for label,color in meta['colors'].items():
 ...
 channels={'clr':rgba,'nrm':(128,128,255,255),'orm':(255,round(.55*255),round(.28*255),255),'msk':(0,0,0,0)}
...
# Drive keeps original reviewed texture maps and UVs.
drive_alias=next(x for x in info['materials']if x.endswith('_storm_drive'));drive_source=BASE/'mesh_materials/expanse15_truman_hull_truman_material.mesh_material';drive_mat=read(drive_source);write(GAME/'mesh_materials'/(drive_alias+'.mesh_material'),drive_mat)
for k,v in drive_mat.items():
 if k.endswith('_texture'):(GAME/'textures'/(v+'.dds')).write_bytes((BASE/'textures'/(v+'.dds')).read_bytes())
