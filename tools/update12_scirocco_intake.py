...
ROOT=Path(__file__).resolve().parents[1];src=Path('/home/haker/Downloads/xxx_-_scirocco_whole.stl');out=ROOT/'assets/derived/update12-b';audit=ROOT/'audit/update12-b'
out.mkdir(parents=True,exist_ok=True);audit.mkdir(parents=True,exist_ok=True);(ROOT/'assets/original/update12-scirocco').mkdir(parents=True,exist_ok=True)
b=src.read_bytes();sha=hashlib.sha256(b).hexdigest();master=ROOT/'assets/original/update12-scirocco'/src.name
