...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update14-scirocco';BUILD=ROOT/'build/update14-scirocco';AUD=ROOT/'audit/update14-scirocco';MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
...
# Keep every child mesh and all materials/textures byte-identical to update13.
BASE=MAIN/'build/experiments/expanse_update13'
for kind in ('rail_0','pdc_base','pdc_barrel'):
 src=BASE/'meshes'/('expanse12_scirocco_'+kind+'.mesh')
 shutil.copyfile(src,BUILD/'game/meshes'/src.name)
meta['status']='PASS OFFLINE HULL; EXISTING MATERIALS AND CHILDREN RETAINED'
write(AUD/'integration-spec.json',meta)
