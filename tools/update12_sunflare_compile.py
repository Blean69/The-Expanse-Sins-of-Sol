...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update12-c';BUILD=ROOT/'build/update12-c';AUD=ROOT/'audit/update12-c';MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
...
for kind in ['hull']:
 name='expanse12_sunflare_'+kind;inp=OUT/(name+'.gltf')
...
# Adapt actual glTF textures to the installed material channels, preserving UVs.
from PIL import Image
MASTER=ROOT/'assets/source/update12-sunflare';asset=Gltf(next(MASTER.glob('*.gltf')));mat=asset.g['materials'][0];pbr=mat['pbrMetallicRoughness'];tex=OUT/'texture-sources';tex.mkdir(exist_ok=True);dest=BUILD/'game/textures';dest.mkdir(parents=True,exist_ok=True);name='expanse12_sunflare_material'
def img(index):return Image.open(MASTER/asset.g['images'][asset.g['textures'][index]['source']]['uri']).convert('RGBA').resize((2048,2048),Image.Resampling.LANCZOS)
img(pbr['baseColorTexture']['index']).save(tex/(name+'_clr.png'));orm=np.array(img(pbr['metallicRoughnessTexture']['index']));orm[:,:,0]=255;orm[:,:,3]=255;Image.fromarray(orm).save(tex/(name+'_orm.png'));normal=np.array(img(mat['normalTexture']['index'])).astype(float)[:,:,:3]/127.5-1;normal/=np.maximum(np.linalg.norm(normal,axis=2,keepdims=True),1e-10);Image.fromarray(np.clip(np.rint((normal+1)*127.5),0,255).astype('uint8')).save(tex/(name+'_nrm.png'));em=np.array(img(mat['emissiveTexture']['index']));mask=np.zeros_like(em);mask[:,:,2]=em[:,:,:3].max(2);Image.fromarray(mask).save(tex/(name+'_msk.png'));material={'version':1,'base_color_texture':name+'_clr','occlusion_roughness_metallic_texture':name+'_orm','normal_texture':name+'_nrm','mask_texture':name+'_msk','emissive_factor':1.0};write(OUT/'materials'/(name+'.mesh_material'),material)
for inp in tex.glob('*.png'):
 ...
 with (BUILD/('texture-'+inp.stem+'.log')).open('w')as log:subprocess.run(cmd,env=env,stdout=log,stderr=subprocess.STDOUT,check=True,timeout=180)
for record in records:
 for alias in record['materials']:assert alias.endswith('_'+name);write(BUILD/'game/mesh_materials'/(alias+'.mesh_material'),material)
for p in dest.glob('*.dds'):assert p.read_bytes()[:4]==b'DDS '
meta['status']='PASS OFFLINE MESHES AND MATERIALS';write(AUD/'integration-spec.json',meta);write(AUD/'material-validation.json',{'status':'PASS OFFLINE','source_dimensions':[4096,4096],'game_dimensions':[2048,2048],'normal_map':'Source referenced glTF normals retained and renormalized after resize; DirectX filename ambiguity requires runtime bump inspection, no unverified green reversal','ORM':'R255 (no AO source), G/B source glTF roughness/metallic','mask':'R/G/A0, B=max source emissiveRGB. Emission is base-color-tinted by installed shader, not independent source emissionRGB','textures':{p.name:hashlib.sha256(p.read_bytes()).hexdigest()for p in dest.glob('*.dds')},'runtime':'NOT RUN'})
