...
ROOT=Path(__file__).resolve().parents[1];GAME=ROOT/'build/update26-storm/game';AUD=ROOT/'audit/update26-storm';meta=read(AUD/'integration-spec.json');parts=[]
...
add(meta['hull_mesh'],[0,0,0]);hull=np.concatenate([p[0]for p in parts]);ray=Rays(hull,ROOT/'build/update26-storm/rays');checks=[]
for rig in meta['rigs']:
 mount={**rig,'weapon_position':rig['position']};safe={'yaw_arc':rig['yaw_arc'],'pitch_arc':rig['pitch_arc']}
...
ray.close()
for label,B in [('oblique',[[.65,0,.76],[-.3,.92,.26],[-.7,-.39,.60]]),('side',[[0,0,1],[0,1,0],[-1,0,0]]),('top',[[0,0,1],[1,0,0],[0,1,0]]),('stern',[[1,0,0],[0,1,0],[0,0,-1]])]:render(parts,(1400,800),np.array(B),fill=.86).save(AUD/(label+'.png'))
meta['arc_checks']=checks;meta['assembled_triangles']=len(hull)+6*(read_mesh(GAME/'meshes/expanse24_storm_pdc_base.mesh')['triangles']+read_mesh(GAME/'meshes/expanse24_storm_pdc_barrel.mesh')['triangles']);meta['adaptations'].append('Frozen firing arcs preserved exactly and checked against the new hull/drive at1-degree samples plus3-degree aim envelope. Static checks do not prove runtime targeting.');write(AUD/'integration-spec.json',meta);print('PASS arcs',sum(x['rays']for x in checks),'rays; actual compiled preview')
