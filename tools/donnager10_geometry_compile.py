...
p=argparse.ArgumentParser();p.add_argument('--sdk',type=Path,required=True);p.add_argument('--wine',type=Path,required=True);p.add_argument('--mesh');args=p.parse_args();out=ROOT/'assets/derived/donnager10-b';build=ROOT/'build/donnager10-b';prefix=build/'proton-prefix';build.mkdir(parents=True,exist_ok=True)
...
for inp in sorted(out.glob('expanse10_*.gltf')):
