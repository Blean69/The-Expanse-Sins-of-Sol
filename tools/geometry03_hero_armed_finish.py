...
out=ROOT/'assets/derived/geometry03-b/hero-armed';build=ROOT/'build/geometry03-b/hero-armed';audit=ROOT/'audit/geometry03-b';sources=read(out/'retained-source-frames.json');meta=read(audit/'hero-armed-mount-metadata.json');dest=build/'game/meshes';jdest=build/'repaired-json';dest.mkdir(parents=True,exist_ok=True);jdest.mkdir(exist_ok=True)
...
original=np.concatenate([np.array(r['positions']) for r in sources['hero_armed'].values()]);actual=np.concatenate(allworld);assert len(actual)//3==meta['triangle_total'];d,_=cKDTree(original).query(actual);d2,_=cKDTree(actual).query(original);assert max(d.max(),d2.max())<2e-5
...
hullpts=records['hero_armed']['meshpoints'];rotation_errors=[]
for r in meta['rigs']:
...
meta['status']='OFFLINE ARMED STATIC HERO VISUAL CHECKS PASS; no functional hero turret/deployment integration; RUNTIME NOT RUN';meta['outputs']=records;meta['rest_pose_position_max_error']=float(max(d.max(),d2.max()));meta['rotation_checks']=rotation_errors;meta['tangent_repair']='Exact16-byte tangent-field substitution only; official regenerated winding/index/grid/trailer preserved';write(audit/'hero-armed-mount-metadata.json',meta);write(audit/'hero-armed-output-validation.json',{'status':'PASS OFFLINE ONLY','triangle_total':len(actual)//3,'rest_pose_position_max_error':meta['rest_pose_position_max_error'],'outputs':records,'rotation_checks':rotation_errors,'runtime':'NOT RUN'})
