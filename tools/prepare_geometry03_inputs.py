...
def main():
    ...
    meta=read(a.worker/'audit/geometry03-b/mount-metadata.json');checks=read(a.worker/'audit/geometry03-b/output-validation.json')
    require(checks['status']=='PASS OFFLINE ONLY' and checks['triangle_total']==16335,'Geometry worker checks incomplete')
    spec={'status':'PASS','hull_mesh':'expanse03_hull','mounts':[r['mount'] for r in meta['rigs']],
          'meshes':[],'worker_report':str((a.worker/'audit/geometry03-b/output-validation.json').resolve())}
    records=[]
    ...
    require(len(records)==13 and sum(r['triangles'] for r in records)==16335,'Missing meshes or changed triangle budget')
    write(ROOT/'build/combat03-inputs/geometry/integration-spec.json',spec)
    write(ROOT/'audit/combat03/integrator-mesh-check.json',{'status':'PASS','meshes':records,'runtime':'NOT RUN'})
