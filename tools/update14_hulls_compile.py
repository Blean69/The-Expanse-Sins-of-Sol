...
import argparse
ap=argparse.ArgumentParser();ap.add_argument('--variant',required=True,choices=['raptor','pella']);args=ap.parse_args();KIND=args.variant;PREFIX=('expanse11_'if KIND=='morrigan'else'expanse12_')+KIND
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update14-hulls'/KIND;BUILD=ROOT/'build/update14-hulls'/KIND;AUD=ROOT/'audit/update14-hulls'/KIND;MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
...
for kind in ['hull']:
 ...
 for fmt in ['json']:
  dest=BUILD/('compiler-'+fmt);dest.mkdir(exist_ok=True);cmd=[str(wine),str(SDK/'MeshBuilder/bin/MeshBuilder.exe'),'--input_path=Z:'+str(inp),'--output_folder_path=Z:'+str(dest),'--mesh_output_format='+fmt]+(['--fill_triangle_facing_grid']if fmt=='binary'else[])
  with (BUILD/(name+'-'+fmt+'.log')).open('a') as log:
   existing=dest/(name+'.mesh_json')
   if not existing.exists() or existing.stat().st_mtime < (OUT/(name+'.bin')).stat().st_mtime:subprocess.run(cmd,env=env,stdout=log,stderr=subprocess.STDOUT,check=True,timeout=1200)
 ...
 if flipped:(OUT/(name+'.bin')).write_bytes(buffer)
 for fmt in (['json','binary']if flipped else['binary']):
  cmd=[str(wine),str(SDK/'MeshBuilder/bin/MeshBuilder.exe'),'--input_path=Z:'+str(inp),'--output_folder_path=Z:'+str(BUILD/('compiler-'+fmt)),'--mesh_output_format='+fmt]+(['--fill_triangle_facing_grid']if fmt=='binary'else[])
  with (BUILD/(name+'-winding-'+fmt+'.log')).open('w')as log:subprocess.run(cmd,env=env,stdout=log,stderr=subprocess.STDOUT,check=True,timeout=1200)
