...
if __name__=='__main__':
    ...
    suffix='_sandbox' if a.sandbox else '';base=ROOT/'build/experiments'/('expanse_update22_1'+suffix);out=ROOT/'build/experiments'/('expanse_update23_2'+suffix);audit=ROOT/'audit'/('update23_2'+suffix)
    ...
    meta=read(base/'.mod_meta_data');meta.update(display_name='The Expanse — 0.23.2 Murphy '+('Sandbox' if a.sandbox else 'Prototype'),display_version='0.23.2',short_description='Separate UNN escort addition over corrected Stage3.',long_description='Includes0.22.1 constructor-menu fix. Adds one UNN Murphy escort with compiled fan hull, four tracking PDCs, bounded fore/aft light rails and existing UNN light torpedo family. Does not implement Stage4/5 recovery/ProtoTech. Offline validation only; runtime NOT RUN.')
