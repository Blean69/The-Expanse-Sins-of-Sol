...
for error,normalweight,options in [(.005,.1,32),(.01,.1,32),(.005,.05,32),(.01,.2,32)]:
 total=0;parts=[]
 for ni,n in enumerate(a.g['nodes']):
  if 'mesh' not in n or ni in [67,68]:continue
  for p in a.g['meshes'][n['mesh']]['primitives']:
   pos=np.ascontiguousarray(a.accessor(p['attributes']['POSITION']),dtype='float32');normal=np.ascontiguousarray(a.accessor(p['attributes']['NORMAL']),dtype='float32');idx=np.ascontiguousarray(a.accessor(p['indices']).flatten(),dtype='uint32');dst=np.zeros_like(idx);weights=np.array([normalweight]*3,dtype='float32');err=c.c_float();nn=fn(dst.ctypes.data_as(u),idx.ctypes.data_as(u),len(idx),pos.ctypes.data_as(f),len(pos),12,normal.ctypes.data_as(f),12,weights.ctypes.data_as(f),3,None,max(12,len(idx)//60*3),error,options,c.byref(err));total+=nn//3;parts.append({'node':ni,'triangles':nn//3,'error':err.value})
...
write(ROOT/'audit/geometry04-b/quality-probe-bounded.json',records)
