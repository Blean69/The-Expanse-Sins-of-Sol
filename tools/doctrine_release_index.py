...
def build():
    rows=[]
    for version in ['19','20','21','22_1','23_2','24','25','26','27','27_1']:
        for suffix in ['', '_sandbox']:
            ...
            evidence='User reported 3-hour playtest' if version=='19' else ('User screenshots: menu/art issues; multiplayer unverified' if version=='25' else ('User reported crash / invalid tag registry; superseded by0.27.1' if version=='27' else 'NOT RUN'))
    ...
    text=['# Fleet Doctrine — ordered local candidates','',
      'The confirmed three-hour session used **0.19 Fleet Balance**. Later user screenshots document0.25 menu/art problems; they are not multiplayer acceptance evidence.0.27.1 is the repair candidate over0.27. The user reported a crash on0.27; it is superseded for play, with0.26 retained as fallback. The user confirmed receiving and equipping the separate0.26 currency-probe item; save/reload and zero-balance rejection remain unconfirmed. All original packages are retained; each ZIP is standalone. Use one variant in a fresh game.','',
    ...
    text+=['','## First test','',
      ...
      '0.27.1 repairs the runtime tag overflow, missing PDC sockets and missing experience effects; post-fix runtime NOT RUN.', '',
