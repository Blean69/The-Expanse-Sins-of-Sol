...
def build():
    rows=[]
    for version in ['19','20','21','22_1','23_2','24','25','26']:
    ...
    text=['# Fleet Doctrine — ordered local candidates','',
      'The confirmed three-hour session used **0.19 Fleet Balance**. Later user screenshots document0.25 menu/art problems; they are not multiplayer acceptance evidence.0.26 is the cumulative correction candidate, with0.25 retained as rollback. All original packages are retained; each ZIP is standalone. Use one variant in a fresh game.','',
    ...
    if probe.exists():
        text+=['## Separate optional currency test','',
          '[0.26 Composite currency probe](expanse_update26_composite_probe.zip) — laboratory only, enable alone. Tests receive1/save/spend1/reject0 using a distinct native exotic and an inert item; no plating. Runtime NOT RUN.',
          '', '`'+sha256(probe)+'`', '']
