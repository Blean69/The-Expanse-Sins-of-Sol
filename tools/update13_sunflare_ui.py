"""Render actual selected Sunflare hull and mounted equipment into static game UI.
...
def main():
    ...
    root=Path(__file__).resolve().parents[1];out=root/'build/update13-c/ui';generated=out/'generated';source_dir=out/'source'
    if out.exists():raise FileExistsError('Refusing to overwrite existing UI derivative: '+str(out))
    if not args.source.is_file():raise FileNotFoundError('Missing actual selected Sunflare source: '+str(args.source))
    ...
    for index,node in enumerate(asset.g['nodes']):
        ...
        for primitive in asset.g['meshes'][node['mesh']]['primitives']:
            ...
            if 'baseColorTexture' in pbr:
                ...
                if not color_path.is_file():raise FileNotFoundError('Missing actual Sunflare material color: '+str(color_path))
    ...
    if triangle_count!=args.expected_triangles:raise ValueError('Unexpected selected Sunflare source geometry count: '+str(triangle_count))
    right=np.array([-.3,0,-.954]);right/=np.linalg.norm(right)
    up=np.array([-.55,.82,.173]);up-=right*np.dot(up,right);up/=np.linalg.norm(up)
    basis=np.array([right,up,np.cross(right,up)])
    portrait=render(meshes,(1836,864),basis)
    source_dir.mkdir(parents=True,exist_ok=True);portrait.save(source_dir/'sunflare_portrait_master.png')
    mask,coords=silhouette(meshes,(1000,480));mask.save(source_dir/'sunflare_silhouette_master.png')
    with (source_dir/'sunflare_silhouette.svg').open('w') as stream:
        stream.write('<svg xmlns="http://www.w3.org/2000/svg" width="1000" height="480" viewBox="0 0 1000 480"><title>Actual Sunflare selected mesh projected triangle silhouette</title><g fill="white">\n')
    ...
    for role in ROLES:
        name='expanse12_sunflare_'+role;brush={'supported_dpis':[150,200],'normal_state':{'texture':name}}
        ...
        for dpi,suffix in [(100,''),(150,'150'),(200,'200')]:
            original=args.game/'textures'/f'trader_scout_corvette_{role}{suffix}.png';size=Image.open(original).size
            if role.startswith('main_view_icon'):
                multiplier=8;base_size=tuple(x*multiplier for x in Image.open(args.game/'textures'/f'trader_scout_corvette_main_view_icon{suffix}.png').size);large_size=tuple(x*multiplier for x in size)
    ...
    for kind in ('small','large'):
        ...
        scaled=portrait.copy();scaled.thumbnail(size,Image.Resampling.LANCZOS);logo=Image.new('RGBA',size,(14,23,35,255));logo.alpha_composite(scaled,((size[0]-scaled.width)//2,(size[1]-scaled.height)//2));name=f'expanse12_sunflare_mod_{kind}_logo.png';logo.save(generated/name);logos[kind+'_logo']=name
    ...
    patches=[]
    for key,role in {'hud_icon':'hud_icon','hud_monochrome_icon':'main_view_icon','hud_picture':'hud_picture','tooltip_picture':'tooltip_picture'}.items():patches.append({'pointer':'/skin_stages/0/gui/'+key,'value':'expanse12_sunflare_'+role})
    for key,role in [('icon','main_view_icon'),('selected_icon','main_view_icon_selected'),('sub_selected_icon','main_view_icon_sub_selected')]:patches.append({'pointer':'/skin_stages/0/main_view_icon/'+key,'value':'expanse12_sunflare_'+role})
    for path,sha in deps.items():
        if digest(path)!=sha:raise ValueError('Sunflare source changed during render: '+path)
    metadata={'game_directory':str(generated),'status':'PASS OFFLINE UI','logos':logos,'source':'Actual selected Sunflare final active source geometry; no other ship imagery','copy_generated_subdirectories':['brushes','textures'],'optional_root_logos':logos,'skin_patches':patches,'entity_manifests_required':[],'runtime':'NOT RUN'}
    ...
    for role in ROLES:
        sprite=Image.open(texdir/f'expanse12_sunflare_{role}200.png')
    ...
    canvas.convert('RGB').save(out/'sunflare-ui-contact-sheet.png')
    audit=root/'audit/update13-c';write(audit/'ui-validation.json',validation);write(audit/'ui-integration-spec.json',metadata);write(audit/'ui-render-recipe.json',recipe)
