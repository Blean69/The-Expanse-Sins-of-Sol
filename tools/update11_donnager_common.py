from common import *
import hashlib
SRC=Path('/run/media/haker/NVME 2/expanse-workers/validation/assets/derived/donnager04-c/master');SOURCE=SRC/'scene.gltf';a=Gltf(SOURCE);OUT=ROOT/'assets/derived/update11-b';AUDIT=ROOT/'audit/update11-b';BUILD=ROOT/'build/update11-b';INTAKE=Path('/run/media/haker/NVME 2/expanse-mod/audit/donnager04-c');old=read(INTAKE/'scale-and-pdc-donor.json');scale=old['provisional_lore_scale']['Donnager_game_length']/old['Donnager_source_length_exporter_units'];center=np.array([-.000046789646,42.71978135,274.714836017]);target=np.zeros(3)
def source_parts():
...
def mesh_name(kind):
 return ("expanse10_" if kind.startswith("donnager_pdc") else "expanse11_")+kind

FROZEN=Path("/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc")
