...
out=ROOT/'assets/derived/amun06-b';build=ROOT/'build/amun06-b';audit=ROOT/'audit/amun06-b';sources=read(out/'retained-source-frames.json');meta=read(audit/'mount-metadata.json');dest=build/'game/meshes';jdest=build/'repaired-json';dest.mkdir(parents=True,exist_ok=True);jdest.mkdir(exist_ok=True)
sha=lambda b:hashlib.sha256(b).hexdigest();records={};allworld=[]
for kind,frame in meta['frames'].items():
 name='expanse06_'+kind;jp=build/'compiler-json'/(name+'.mesh_json');bp=build/'compiler-binary'/(name+'.mesh');m=read(jp);original=bp.read_bytes();b=bytearray(original);count=struct.unpack_from('<Q',b,53)[0];off=61;binary=[];tangent_offsets=[]
...
hullpts=records['amun_hull']['meshpoints'];rotation_errors=[]
for r in meta['rigs']:
...
dr.text((25,18),'AMUN THREE AIMING RIG CANDIDATE / RUNTIME NOT RUN',fill='white');dr.text((25,44),'Three independent yaw/pitch rigs; rotation runtime NOT RUN. Left ship X/Z; right ship Y/Z.',fill='white');dr.text((25,850),f'Amun plus separate pod: {meta["triangle_total"]} triangles. Three actual source joints; runtime aiming and pod visual NOT RUN.',fill='white');im.save(audit/'compiled-preview.png')
