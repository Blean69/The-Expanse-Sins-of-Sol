...
p=argparse.ArgumentParser();p.add_argument('--sdk',type=Path,required=True);p.add_argument('--wine',type=Path,required=True);args=p.parse_args();out=ROOT/'assets/derived/amun06-b';build=ROOT/'build/amun06-b';prefix=build/'proton-prefix';build.mkdir(parents=True,exist_ok=True)
...
for inp in sorted(out.glob('expanse06_*.gltf')):
