...
PREFIX='expanse19_europa'
from common import read_mesh,Gltf
from polish_ui import write
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update19-europa';BUILD=ROOT/'build/update19-europa';AUD=ROOT/'audit/update19-europa';MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
...
for kind in ['hull','pdc_base','pdc_barrel']:
 name=PREFIX+('_bane_hull'if kind=='hull'else'_'+kind);inp=OUT/(name+'.gltf')
