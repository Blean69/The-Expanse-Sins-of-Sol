...
out=ROOT/'assets/derived/torpedo05-b';build=ROOT/'build/torpedo05-b';audit=ROOT/'audit/torpedo05-b';sources=read(out/'retained-source-frames.json');meta=read(audit/'mount-metadata.json');dest=build/'game/meshes';jdest=build/'repaired-json';dest.mkdir(parents=True,exist_ok=True);jdest.mkdir(exist_ok=True)
sha=lambda b:hashlib.sha256(b).hexdigest();records={};allworld=[]
for kind,frame in meta['frames'].items():
 name='expanse05_'+kind;jp=build/'compiler-json'/(name+'.mesh_json');bp=build/'compiler-binary'/(name+'.mesh');m=read(jp);original=bp.read_bytes();b=bytearray(original);count=struct.unpack_from('<Q',b,53)[0];off=61;binary=[];tangent_offsets=[]
...
hullpts=records['amun_torpedo']['meshpoints'];rotation_errors=[]
for r in meta['rigs']:
...
dr.text((25,18),'TORPEDO VISUAL CANDIDATE / RUNTIME NOT RUN',fill='white');dr.text((25,44),'Nose+Z; measured aft exhaust-Z; runtime NOT RUN. Left ship X/Z; right ship Y/Z.',fill='white');dr.text((25,850),f'Isolated torpedo: {meta["triangle_total"]} triangles. Nose+Z; exhaust-Z. Appearance and interception runtime NOT RUN.',fill='white');im.save(audit/'torpedo-preview.png')
