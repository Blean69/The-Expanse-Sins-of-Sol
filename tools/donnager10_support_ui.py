...
def main():
    ...
    root=Path(__file__).resolve().parents[1];out=root/'build/donnager10-c/ui';generated=out/'generated';source_dir=out/'source'
    ...
    for role in ROLES:
        name='expanse10_donnager_'+role;brush={'supported_dpis':[150,200],'normal_state':{'texture':name}}
    ...
    for kind in ('small','large'):
        ...
        scaled=portrait.copy();scaled.thumbnail(size,Image.Resampling.LANCZOS);logo=Image.new('RGBA',size,(14,23,35,255));logo.alpha_composite(scaled,((size[0]-scaled.width)//2,(size[1]-scaled.height)//2));name=f'expanse10_donnager_mod_{kind}_logo.png';logo.save(generated/name);logos[kind+'_logo']=name
    ...
    patches=[]
    for key,role in {'hud_icon':'hud_icon','hud_monochrome_icon':'main_view_icon','hud_picture':'hud_picture','tooltip_picture':'tooltip_picture'}.items():patches.append({'pointer':'/skin_stages/0/gui/'+key,'value':'expanse10_donnager_'+role})
    for key,role in [('icon','main_view_icon'),('selected_icon','main_view_icon_selected'),('sub_selected_icon','main_view_icon_sub_selected')]:patches.append({'pointer':'/skin_stages/0/main_view_icon/'+key,'value':'expanse10_donnager_'+role})
    ...
    for role in ROLES:
        sprite=Image.open(texdir/f'expanse10_donnager_{role}200.png')
    ...
    audit=root/'audit/donnager10-c';write(audit/'ui-validation.json',validation);write(audit/'ui-integration-spec.json',metadata);write(audit/'ui-render-recipe.json',recipe)
