...
p=argparse.ArgumentParser();p.add_argument('--sdk',type=Path,required=True);p.add_argument('--wine',type=Path,required=True);p.add_argument('--mesh');args=p.parse_args();out=ROOT/'assets/derived/update11-b';build=ROOT/'build/update11-b';prefix=build/'proton-prefix';build.mkdir(parents=True,exist_ok=True)
if not prefix.exists():shutil.copytree(FROZEN/'build/polish-b/proton-prefix',prefix,symlinks=True)
...
for inp in sorted(out.glob('expanse*.gltf')):
 if 'editable' in inp.name or (args.mesh and inp.stem!=args.mesh):continue
