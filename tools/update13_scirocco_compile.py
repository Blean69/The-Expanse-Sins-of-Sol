...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update13-a';BUILD=ROOT/'build/update13-a';AUD=ROOT/'audit/update13-a';MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
...
# The rail is an unchanged validated asset. Reuse its exact original binary.
original_rail=DONOR/'build/update12-b/game/meshes/expanse12_scirocco_rail_0.mesh'
shutil.copyfile(original_rail,BUILD/'game/meshes'/original_rail.name)
old_report=json.loads((DONOR/'audit/update12-b/mesh-validation.json').read_text())
records=[next(r for r in old_report['meshes']if r['mesh']==x['mesh'])if x['mesh'].endswith('rail_0')else x for x in records]
write(AUD/'mesh-validation.json',{'status':'PASS OFFLINE','meshes':records,'runtime':'NOT RUN'})
# New generated panel albedo; other actual map channels remain pinned existing assets.
base=DONOR/'build/update12-b/game'
dest=BUILD/'game/textures';dest.mkdir(parents=True,exist_ok=True)
for src in (base/'textures').glob('*.dds'):shutil.copyfile(src,dest/src.name)
for src in (base/'mesh_materials').glob('*.mesh_material'):
 data=json.loads(src.read_text())
 if '_mat_0' in src.stem:data['base_color_texture']='expanse13_scirocco_panels_clr'
 write(BUILD/'game/mesh_materials'/src.name,data)
inp=OUT/'texture-sources/expanse13_scirocco_panels_clr.png'
cmd=[str(wine),str(MAIN/'.tools/texconv.exe'),'-f','BC7_UNORM','-y','-m','0','-nogpu','--single-proc','-bc','q','-o','Z:'+str(dest),'Z:'+str(inp)]
with (BUILD/'texture-panels.log').open('w')as log:subprocess.run(cmd,env=env,stdout=log,stderr=subprocess.STDOUT,check=True,timeout=120)
meta['status']='PASS OFFLINE MESHES AND MATERIALS';write(AUD/'integration-spec.json',meta)
