"""Render actual selected Europa hull and mounted equipment into static game UI.
...
def main():
    ...
    parser.add_argument('--variant',choices=['europa'],default='europa')
    ...
    root=Path(__file__).resolve().parents[1];out=root/'build/update19-europa/ui-final';generated=out/'generated';source_dir=out/'source'
    if out.exists():raise FileExistsError('Refusing to overwrite existing UI derivative: '+str(out))
    if not args.source.is_file():raise FileNotFoundError('Missing actual selected Europa source: '+str(args.source))
    ...
    for index,node in enumerate(asset.g['nodes']):
        ...
        for primitive in asset.g['meshes'][node['mesh']]['primitives']:
            ...
            if 'baseColorTexture' in pbr:
                ...
                if not color_path.is_file():raise FileNotFoundError('Missing actual Europa material color: '+str(color_path))
    ...
    if triangle_count!=args.expected_triangles:raise ValueError('Unexpected selected Europa source geometry count: '+str(triangle_count))
    ...
    with (source_dir/'ship_silhouette.svg').open('w') as stream:
        stream.write('<svg xmlns="http://www.w3.org/2000/svg" width="1000" height="480" viewBox="0 0 1000 480"><title>Actual Europa selected mesh projected triangle silhouette</title><g fill="white">\n')
    ...
    for role in ROLES:
        name='expanse19_'+args.variant+'_'+role;brush={'supported_dpis':[150,200],'normal_state':{'texture':name}}
    ...
    for kind in ('small','large'):
        ...
        scaled=portrait.copy();scaled.thumbnail(size,Image.Resampling.LANCZOS);logo=Image.new('RGBA',size,(14,23,35,255));logo.alpha_composite(scaled,((size[0]-scaled.width)//2,(size[1]-scaled.height)//2));name=f'expanse19_{args.variant}_mod_{kind}_logo.png';logo.save(generated/name);logos[kind+'_logo']=name
    ...
    patches=[]
    for key,role in {'hud_icon':'hud_icon','hud_monochrome_icon':'main_view_icon','hud_picture':'hud_picture','tooltip_picture':'tooltip_picture'}.items():patches.append({'pointer':'/skin_stages/0/gui/'+key,'value':'expanse19_'+args.variant+'_'+role})
    for key,role in [('icon','main_view_icon'),('selected_icon','main_view_icon_selected'),('sub_selected_icon','main_view_icon_sub_selected')]:patches.append({'pointer':'/skin_stages/0/main_view_icon/'+key,'value':'expanse19_'+args.variant+'_'+role})
    for path,sha in deps.items():
        if digest(path)!=sha:raise ValueError('Europa source changed during render: '+path)
    metadata={'game_directory':str(generated),'status':'PASS OFFLINE UI','logos':logos,'source':'Actual selected Europa final active source geometry; no other ship imagery','copy_generated_subdirectories':['brushes','textures'],'optional_root_logos':logos,'skin_patches':patches,'entity_manifests_required':[],'runtime':'NOT RUN'}
    ...
    for role in ROLES:
        sprite=Image.open(texdir/f'expanse19_{args.variant}_{role}200.png')
    ...
    audit=root/'audit/update19-europa';write(audit/'ui-validation.json',validation);write(audit/'ui-integration-spec.json',metadata);write(audit/'ui-render-recipe.json',recipe)
