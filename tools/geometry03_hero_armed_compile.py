...
p=argparse.ArgumentParser();p.add_argument('--sdk',type=Path,required=True);p.add_argument('--wine',type=Path,required=True);args=p.parse_args();out=ROOT/'assets/derived/geometry03-b/hero-armed';build=ROOT/'build/geometry03-b/hero-armed';prefix=build/'proton-prefix';build.mkdir(parents=True,exist_ok=True)
...
for inp in sorted(out.glob('expanse03_*.gltf')):
