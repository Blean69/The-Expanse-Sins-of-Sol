...
p=argparse.ArgumentParser();p.add_argument('--sdk',type=Path,required=True);p.add_argument('--wine',type=Path,required=True);args=p.parse_args();out=ROOT/'assets/derived/torpedo05-b';build=ROOT/'build/torpedo05-b';prefix=build/'proton-prefix';build.mkdir(parents=True,exist_ok=True)
...
for inp in sorted(out.glob('expanse05_*.gltf')):
