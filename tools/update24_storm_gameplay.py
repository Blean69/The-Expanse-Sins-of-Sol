...
def changes(base=BASE):
 ...
 d=get('expanse12_raptor','unit');d['skin_groups']=[{'skins':[ID]}];d['tags']=['capital_ship',ID];d['target_filter_unit_type']='capital_ship';d['build'].update(build_kind='cruiser',build_group_id='capital_ship',supply_cost=300,build_time=240.,price={'credits':9000.,'metal':3000.,'crystal':2200.},exotic_price=[{'exotic_type':'offense','count':2},{'exotic_type':'utility','count':2},{'exotic_type':'ultimate','count':2}],prerequisites=[[UNLOCK]])
 ...
 base_hull=d['health']['levels'][0]['max_hull_points'];base_armor=d['health']['levels'][0]['max_armor_points']
 for level in d['health']['levels']:
  level['max_hull_points']=round(level['max_hull_points']*6000/base_hull,6);level['max_armor_points']=round(level['max_armor_points']*3000/base_armor,6)
 ...
 # Native jump effect avoids the donor Raptor's absolute multi-nozzle phase plume.
 stage['effects']['hyperspace_effects']=copy.deepcopy(get('trader_skirmisher_corvette_frigate','unit_skin')['skin_stages'][0]['effects']['hyperspace_effects'])
 # Idle exhaust is a single emitter instanced at compiled exhaust.0.
