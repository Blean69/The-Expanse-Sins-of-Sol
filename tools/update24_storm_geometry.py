...
rot=np.array([[0,1,0],[0,0,-1],[-1,0,0]],float);scale=200*(1.875)/np.ptp(src[:,:,0]);tri=(src-center)@rot.T*scale
...
lo=tri.min((0,1));hi=tri.max((0,1));report={'status':'GEOMETRY AUTHORED; COMPILATION PENDING','source':{'zip':str(ZIP),'zip_sha256':hashlib.sha256(ZIP.read_bytes()).hexdigest(),'entry':ENTRY,'entry_sha256':hashlib.sha256(raw).hexdigest(),'author':'dredeth','license':'CC BY-NC 4.0 (archive intake)','original_triangles':len(src),'removed_degenerates':int((~good).sum()),'remaining_triangles':len(tri),'simplification':False},'hull_mesh':'expanse24_storm_hull','chosen_length_metres':200,'game_units_per_metre':1.875,'source_units_are_not_metres':True,'source_to_game_rotation':rot.tolist(),'source_center':center.tolist(),'scale':float(scale),'colors':colors,'meshpoints':points,'rigs':rigs,'fixed_rail':{'mesh_point':'weapon.keel_rail.0','weapon_position':rail.tolist(),'forward':[0,0,1],'yaw_arc':{'min_angle':-2.,'max_angle':2.},'pitch_arc':{'min_angle':-2.,'max_angle':2.},'turret':False},'spatial':{'box':{'center':((hi+lo)/2).tolist(),'extents':((hi-lo)/2).tolist()},'radius':float(np.linalg.norm(tri.reshape(-1,3),axis=1).max())},'runtime':'NOT RUN','adaptations':['200 m design length and six PDC mounts are mod choices, not a verified canonical specification.','Six real rotating donor PDC rigs; fixed source keel rail geometry is not a turret.','Two unmodeled torpedo apertures use actual forward hull surface origins; no invented launcher geometry claim.','Pearl/blue/blush static facet materials approximate crystalline appearance; not an angle-dependent iridescent shader.','Printable source seams remain; no fabricated topology fill.']}
