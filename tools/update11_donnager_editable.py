...
for kind,parts in ss.items():
 mesh={'name':kind,'primitives':[]};inp=read(OUT/(mesh_name(kind)+'.gltf'));aa=Gltf(OUT/(mesh_name(kind)+'.gltf'))
...
g['buffers']=[{'uri':'update11_donnager_editable.bin','byteLength':len(buf)}];(OUT/'update11_donnager_editable.bin').write_bytes(buf);write(OUT/'update11_donnager_editable.gltf',g);write(AUDIT/'editable-contract.json',{'path':str(OUT/'update11_donnager_editable.gltf'),'active_nodes':len(g['nodes']),'unique_meshes':len(g['meshes']),'triangles':m['assembled_triangle_total'],'compiler_z_reflected':False,'coordinate_convention':'+Z bow,+Y up; centered actual source scaled uniformly475.5/46Tachi reference','source_image_dependencies':[str((OUT/i['uri']).resolve()) for i in g['images']]});print('Editable',len(g['nodes']),'nodes',m['assembled_triangle_total'],'triangles')
