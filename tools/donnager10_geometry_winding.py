...
P=argparse.ArgumentParser();P.add_argument('--sdk',type=Path,required=True);P.add_argument('--wine',type=Path,required=True);args=P.parse_args();out=ROOT/'assets/derived/donnager10-b';build=ROOT/'build/donnager10-b';records={}
...
for jp in sorted((build/'compiler-json').glob('*.mesh_json')):
 ...
 for pp in m['primitives']:
  actual_material=m['materials'][pp['material_index']];matches=[q for q in a.g['meshes'][0]['primitives'] if actual_material.endswith('_'+a.g['materials'][q['material']]['name'])];assert len(matches)==1,(name,actual_material);pr=matches[0];start=pp['vertex_index_start'];ii=allidx[start:start+pp['vertex_index_count']].reshape(-1,3);vv=v[ii];face=np.cross(vv[:,1]-vv[:,0],vv[:,2]-vv[:,0]);mean_normal=n[ii].mean(1);dot=np.sum(face*mean_normal,axis=1);cosine=dot/np.maximum(np.linalg.norm(face,axis=1)*np.linalg.norm(mean_normal,axis=1),1e-15);flip=cosine< -1e-5
  ...
  assert max(errors)<4e-5,(name,'triangle order changed',max(errors))
...
write(ROOT/'audit/donnager10-b/winding-correction.json',records)
