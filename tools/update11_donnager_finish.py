...
out=ROOT/'assets/derived/update11-b';build=ROOT/'build/update11-b';audit=ROOT/'audit/update11-b';sources=read(out/'retained-source-frames.json');meta=read(audit/'mount-metadata.json');dest=build/'game/meshes';jdest=build/'repaired-json';dest.mkdir(parents=True,exist_ok=True);jdest.mkdir(exist_ok=True)
sha=lambda b:hashlib.sha256(b).hexdigest();records={};allworld=[]
for kind,frame in meta['frames'].items():
 name=mesh_name(kind);jp=build/'compiler-json'/(name+'.mesh_json');bp=build/'compiler-binary'/(name+'.mesh');m=read(jp);original=bp.read_bytes();b=bytearray(original);count=struct.unpack_from('<Q',b,53)[0];off=61;binary=[];tangent_offsets=[]
...
for k,parts in sources.items():
 inp=Gltf(out/(mesh_name(k)+'.gltf'));unreferenced_source_vertices[k]=0
