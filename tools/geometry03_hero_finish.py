...
out=ROOT/'assets/derived/geometry03-b/hero';build=ROOT/'build/geometry03-b/hero';audit=ROOT/'audit/geometry03-b';sources=read(out/'retained-source-frames.json');meta=read(audit/'hero-mount-metadata.json');dest=build/'game/meshes';jdest=build/'repaired-json';dest.mkdir(parents=True,exist_ok=True);jdest.mkdir(exist_ok=True)
sha=lambda b:hashlib.sha256(b).hexdigest();records={};allworld=[]
for kind,frame in meta['frames'].items():
 name='expanse03_'+kind;jp=build/'compiler-json'/(name+'.mesh_json');bp=build/'compiler-binary'/(name+'.mesh');m=read(jp);original=bp.read_bytes();b=bytearray(original);count=struct.unpack_from('<Q',b,53)[0];off=61;binary=[];tangent_offsets=[]
...
original=np.concatenate([np.array(r['positions']) for r in sources['hero_static'].values()]);actual=np.concatenate(allworld);assert len(actual)//3==meta['triangle_total'];d,_=cKDTree(original).query(actual);d2,_=cKDTree(actual).query(original);assert max(d.max(),d2.max())<2e-5
...
hullpts=records['hero_static']['meshpoints'];rotation_errors=[]
for r in meta['rigs']:
...
meta['status']='OFFLINE STATIC HERO VISUAL CHECKS PASS; no functional hero turret/deployment integration; RUNTIME NOT RUN';meta['outputs']=records;meta['rest_pose_position_max_error']=float(max(d.max(),d2.max()));meta['rotation_checks']=rotation_errors;meta['tangent_repair']='Exact16-byte tangent-field substitution only; official regenerated winding/index/grid/trailer preserved';write(audit/'hero-mount-metadata.json',meta);write(audit/'hero-output-validation.json',{'status':'PASS OFFLINE ONLY','triangle_total':len(actual)//3,'rest_pose_position_max_error':meta['rest_pose_position_max_error'],'outputs':records,'rotation_checks':rotation_errors,'runtime':'NOT RUN'})
...
dr.text((25,18),'HERO STATIC DEPLOYED VISUAL CANDIDATE / RUNTIME NOT RUN',fill='white');dr.text((25,44),'Static deployed geometry; no functional hero rotation rig. Left ship X/Z; right ship Y/Z.',fill='white');dr.text((25,850),f'Stand-free optimized hero: {meta["triangle_total"]} triangles. PDC deployment baked; no runtime turret rig yet.',fill='white');im.save(audit/'hero-static-preview.png')
