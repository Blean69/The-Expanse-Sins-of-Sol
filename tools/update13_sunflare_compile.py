...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update13-c';BUILD=ROOT/'build/update13-c';AUD=ROOT/'audit/update13-c';MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
if not prefix.exists():shutil.copytree(Path('/run/media/haker/NVME 2/expanse-workers/validation/build/update12-c/proton-prefix'),prefix,symlinks=True)
...
# Reuse frozen material/DDS bytes without texture edits.
old=Path('/run/media/haker/NVME 2/expanse-workers/validation/build/update12-c/game')
for folder in ['mesh_materials','textures']:
 shutil.copytree(old/folder,BUILD/'game'/folder,dirs_exist_ok=True)
