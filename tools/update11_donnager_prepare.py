...
donor=read(FROZEN/'audit/polish-b/mount-metadata.json')['rigs'][0]['turret_override'];pdcoffset=np.array(donor['barrel_position']);pdcmuzzle=np.array(donor['muzzle_positions'][0])
for kind,part in [('donnager_pdc_base','base'),('donnager_pdc_barrel','barrel')]:
 inp=Gltf(FROZEN/'assets/derived/polish-b'/f'expanse_polish_pdc_0_{part}.gltf');frames[kind]={'basis':np.eye(3).tolist(),'origin':[0,0,0]}
...
for r in layout['rails']:
 i=r['index'];p=np.array(r['pivot']);kind=f'donnager_rail_{i}';frames[kind]={'basis':np.eye(3).tolist(),'origin':p.tolist()};name=mesh_name(kind)
 ...
 mount={'weapon':f'expanse10_donnager_rail_{i}','mesh_point':f'child.expanse10_donnager_rail_{i}','weapon_position':p.tolist(),'up':[0,1,0],'forward':[0,0,1],'yaw_arc':{'min_angle':-2.,'max_angle':2.},'pitch_arc':{'min_angle':0.,'max_angle':0.}}
...
meshpoints={'donnager_hull':points};materialspec=[{'name':f'expanse11_donnager_mat_{i}','original':copy.deepcopy(mat)} for i,mat in enumerate(a.g['materials'])];materialspec.append({'name':'mcrn_tachi_material','original':{'pbrMetallicRoughness':{'baseColorTexture':{'index':2}}}});counts={};sourceframes={};flatfallback=0
for kind,frame in frames.items():
 B=np.array(frame['basis']);origin=np.array(frame['origin']);name=mesh_name(kind);g={'asset':{'version':'2.0','generator':'Measured Donnager prototype; official compiler Z compensation'},'scene':0,'scenes':[{'nodes':[0]}],'nodes':[{'name':name,'mesh':0,'children':[]}],'meshes':[{'name':name,'primitives':[]}],'buffers':[],'bufferViews':[],'accessors':[],'materials':[]};buf=bytearray();sourceframes[kind]={};counts[kind]=0
