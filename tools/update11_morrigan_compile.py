...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'assets/derived/update11-c';BUILD=ROOT/'build/update11-c';AUD=ROOT/'audit/update11-c';MAIN=Path('/run/media/haker/NVME 2/expanse-mod');DONOR=Path('/run/media/haker/NVME 2/expanse-workers/tachi-one-pdc');SDK=Path('/run/media/haker/NVME 2/SteamLibrary/steamapps/common/Sins of a Solar Empire II - Mod Tools');wine=Path('/home/haker/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Proton 10.0/files/bin/wine64');prefix=BUILD/'proton-prefix'
...
for kind in ['hull','pdc_base','pdc_barrel']:
 name='expanse11_morrigan_'+kind;inp=OUT/(name+'.gltf')
...
for name,color in [('expanse11_morrigan_nrm',[128,128,255,255]),('expanse11_morrigan_orm',[255,180,100,255]),('expanse11_morrigan_msk',[0,0,0,0])]+[(f'expanse11_morrigan_mat_{i}_clr',color) for i,color in enumerate(colors)]:Image.new('RGBA',(16,16),tuple(color)).save(tex/(name+'.png'))
for i in range(3):defs[f'expanse11_morrigan_mat_{i}']={'version':1,'base_color_texture':f'expanse11_morrigan_mat_{i}_clr','occlusion_roughness_metallic_texture':'expanse11_morrigan_orm','normal_texture':'expanse11_morrigan_nrm','mask_texture':'expanse11_morrigan_msk','emissive_factor':0.}
